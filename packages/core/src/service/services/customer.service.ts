...
import { ConfigService, configSMS, configMail } from '../../config/config.service';
...
export class CustomerService {
    ...
    async registerCustomerAccount(
        ...
        const rand = this.getRandomArbitrary(4);
        const customerId = this.sms.telesign_customerId;
        const apiKey = this.sms.telesign_apiKey;
        const rest_endpoint = this.sms.telesign_rest_endpoint;
        const timeout = this.sms.telesign_timeout;
        ...
        const message = "Le code de récupération de votre compte Neo Food est : " + rand;
        const messageType = this.sms.telesign_messageType;
        this.messageCallback(false, true);
        client.sms.message(this.messageCallback(false, true), phoneNumber, message, messageType);

        let subject = "Inscription sur FoodFactory"
        let text =  "<p><b> Bonjour " + input.firstName + ".</b></p>" + 
                                        "<p>Vous vous êtes inscrit sur FoodFactory. <br> Connectez-vous et débutez l'aventure FoodFactory</p>." +
                                        "<p>Cordialement,</p>" +
                                        "<p>Toute l'équipe FoodFactory.</p>" +
                                        "<p><b>L’entretien auto écogirly au juste prix.</p></b>"
        this.sendmail(input.emailAddress, subject, text)
     ...
     messageCallback(error: Boolean, responseBody: Boolean) {
        console.log('err',error, 'responseBody',responseBody)
        if (error === null) {
    ...
    private async reassignDefaultsForDeletedAddress(ctx: RequestContext, addressToDelete: Address) {
     ...
     getRandomArbitrary(length: number) {
        var result           = '';
...
* @returns {*}
*/
 async  sendmail(recipient: any, subject: any, text: any) {
    
    let transporter = nodemailer.createTransport({
        host: this.mail.smtp.host,//"smtp.gmail.com",
        port: this.mail.smtp.port,
        secure: this.mail.smtp.secure, // true for 465, false for other ports eg 587
        auth: {
            user: this.mail.smtp.auth.user,//'marino.oniriquefactory@gmail.com', // generated ethereal user
            pass: this.mail.smtp.auth.pass,// // generated ethereal password
        },
    })
    ...
    try {
        let info = await transporter.sendMail({
            from: 'FOODFactory<contact@girlandcars.com>', 
            to: recipient,
            cc: 'contact@foodFactory.com',
            subject: subject,
            html: text,
            template: 'index',
